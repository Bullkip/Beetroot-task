...
function filter() {
    ...
    if ($departments) {
        foreach ($departments as $department) {
            if (isset($_POST['department_' . $department->term_id]) && $_POST['department_' . $department->term_id] == 'on') $all_department[] = $department->term_id;
        }
        if (count($all_department) > 0) {
            $main_query[] = array('taxonomy' => 'department', 'field' => 'id', 'terms' => $all_department,);
        }
    }
    // for offices
    if ($offices) {
        foreach ($offices as $office) {
            if (isset($_POST['office_' . $office->term_id]) && $_POST['office_' . $office->term_id] == 'on') $all_offices[] = $office->term_id;
        }
        if (count($all_offices) > 0) {
            $sub_query[] = array('taxonomy' => 'offices', 'field' => 'id', 'terms' => $all_offices,);
        }
    }
    // for offices
    if ($academies) {
        foreach ($academies as $academy) {
            if (isset($_POST['academy_' . $academy->term_id]) && $_POST['academy_' . $academy->term_id] == 'on') $all_academies[] = $academy->term_id;
        }
        if (count($all_academies) > 0) {
            $sub_query[] = array('taxonomy' => 'academies', 'field' => 'id', 'terms' => $all_academies,);
        }
    }
    ...
    $args = array('posts_per_page' => - 1, 'post_type' => 'vacancy', 'orderby' => 'date', 'order' => 'DESC', 'tax_query' => $main_query,);
    global $post;
    $query = new WP_Query;
